...
const TradersDashboard: React.FC<TradersDashboardProps> = (props) => {
  ...
  const [tpStopFilter, setTpStopFilter] = useState<'all' | '2tp' | 'stop'>('all')
  ...
  const handleTraderClick = (traderId: string) => {
    if (props.onTraderClick) {
      props.onTraderClick(traderId)
    } else {
      ...
      <td className="px-4 py-3 text-center">
        <div className="flex space-x-1">
          ...
          <Button 
            size="sm" 
            variant="outline"
            className="bg-blue-600 text-white border-blue-600 hover:bg-blue-700 text-xs px-2 py-1"
            onClick={(e) => {
              e.stopPropagation()
              // Handle TP2 click
          ...
          <Button 
            size="sm" 
            variant="outline"
            className="bg-yellow-500 text-black border-yellow-500 hover:bg-yellow-600 text-xs px-2 py-1"
          >
            🔥 Активен
          </Button>
          <Button 
            ...
            <table className="w-full">
              <thead className="bg-gray-800">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-sm font-medium text-gray-300">Капитал</th>
