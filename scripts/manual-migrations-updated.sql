-- GHOST Manual Migrations (Updated)
-- Примените этот SQL в Supabase SQL Editor
-- Эта версия игнорирует уже существующие политики
...
-- Функция для создания политики только если она не существует
...
SELECT create_policy_if_not_exists(
  'signals',
  'All users can view signals',
  'CREATE POLICY "All users can view signals" ON signals FOR SELECT USING (true)'
);
...
SELECT create_policy_if_not_exists(
  'news_events',
  'All users can view news events',
  'CREATE POLICY "All users can view news events" ON news_events FOR SELECT USING (true)'
);

-- Политики для ИИ анализа
SELECT create_policy_if_not_exists(
  'ai_analysis',
  'All users can view AI analysis',
  'CREATE POLICY "All users can view AI analysis" ON ai_analysis FOR SELECT USING (true)'
);

-- Политики для стратегий
SELECT create_policy_if_not_exists(
  'strategies',
  'Users can manage own strategies',
...
SELECT create_policy_if_not_exists(
  'trade_timeline',
  'Users can view own trade timeline',
  'CREATE POLICY "Users can view own trade timeline" ON trade_timeline FOR SELECT USING (EXISTS (SELECT 1 FROM trades WHERE trades.id = trade_timeline.trade_id AND trades.user_id = auth.uid()))'
);

-- Политики для ценовых данных
SELECT create_policy_if_not_exists(
  'price_feed',
  'All users can view price feed',
  'CREATE POLICY "All users can view price feed" ON price_feed FOR SELECT USING (true)'
);

-- Политики для новых таблиц
SELECT create_policy_if_not_exists(
  'influential_tweets',
  'All users can view influential tweets',
  'CREATE POLICY "All users can view influential tweets" ON influential_tweets FOR SELECT USING (true)'
);

SELECT create_policy_if_not_exists(
  'market_data',
  'All users can view market data',
  'CREATE POLICY "All users can view market data" ON market_data FOR SELECT USING (true)'
);

SELECT create_policy_if_not_exists(
  'analysis_history',
  'Users can view own analysis history',
  'CREATE POLICY "Users can view own analysis history" ON analysis_history FOR SELECT USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'analysis_history',
  'Users can insert own analysis history',
  'CREATE POLICY "Users can insert own analysis history" ON analysis_history FOR INSERT WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'news_bookmarks',
  'Users can manage own news bookmarks',
...
SELECT create_policy_if_not_exists(
  'trading_sessions',
  'Users can manage own trading sessions',
  'CREATE POLICY "Users can manage own trading sessions" ON trading_sessions FOR ALL USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'trading_patterns',
  'Users can manage own trading patterns',
  'CREATE POLICY "Users can manage own trading patterns" ON trading_patterns FOR ALL USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'trading_goals',
  'Users can manage own trading goals',
  'CREATE POLICY "Users can manage own trading goals" ON trading_goals FOR ALL USING (auth.uid() = user_id)'
);
...
DROP TRIGGER IF EXISTS update_trading_patterns_updated_at ON trading_patterns;
CREATE TRIGGER update_trading_patterns_updated_at BEFORE UPDATE ON trading_patterns
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_trading_goals_updated_at ON trading_goals;
