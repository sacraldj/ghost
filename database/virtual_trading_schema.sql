-- Схема таблиц для виртуального трейдинга
-- Создается поверх существующих таблиц v_trades, signals_parsed, signals_raw
...
CREATE TABLE IF NOT EXISTS virtual_positions (
    ...
    -- Связи с сигналами
    signal_id UUID REFERENCES signals_parsed(id),  -- Оригинальный сигнал
    v_trade_id UUID REFERENCES v_trades(id),       -- Связанная виртуальная сделка
    ...
    -- Временные метки
    signal_time TIMESTAMPTZ NOT NULL,              -- Время получения сигнала
...
CREATE TABLE IF NOT EXISTS position_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    position_id UUID REFERENCES virtual_positions(id) ON DELETE CASCADE,
